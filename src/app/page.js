...
const MainLoader = ({ onComplete }) => {
  ...
  useEffect(() => {
    const duration = 3000; // 5-second countdown
    ...
    const timer = setInterval(() => {
      setProgress((prev) => {
        if (prev >= 100) {
          clearInterval(timer);
          setTimeout(onComplete, 500); // Small buffer before showing main site
          return 100;
        }
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-[#8B2510]">
      ...
      <div className="relative flex flex-col items-center gap-8">
        {/* Pulsing Favicon Logo */}
        <motion.div
          animate={{ scale: [1, 1.05, 1], rotate: [0, 5, -5, 0] }}
          transition={{ repeat: Infinity, duration: 2 }}
          className="w-24 h-24 bg-white border-4 border-black rounded-2xl flex items-center justify-center shadow-[8px_8px_0_0_#000]"
        >
          <Image src="/favicon.ico" alt="Logo" width={60} height={60} className="object-contain" />
        </motion.div>

        {/* 5-Second Progress Bar */}
        <div className="flex flex-col items-center gap-4">
          <div className="w-64 h-5 bg-black border-2 border-black rounded-full overflow-hidden shadow-[4px_4px_0_0_rgba(0,0,0,0.3)]">
            <div 
              className="h-full bg-[#EBB048] transition-all duration-75 ease-linear"
              style={{ width: `${progress}%` }}
            />
          </div>
          <div className="flex flex-col items-center">
            <span className="text-[#EBB048] font-black uppercase tracking-[0.3em] text-[10px] italic animate-pulse">
              Syncing PostPilot wait a sec...
            </span>
            <span className="text-white/40 font-mono text-[10px] mt-2">{Math.round(progress)}%</span>
          </div>
        </div>
      </div>
    </div>
...
export default function Home() {
  ...
  // Note: Clerk auth redirects should generally be handled in middleware or 
  // a separate useEffect if using "use client" on the home page.

  if (!isReady) {
    return <MainLoader onComplete={() => setIsReady(true)} />;
  }
    ...
    <div className={`${boko_font.variable} h-screen w-full bg-[#8B2510] relative flex flex-col overflow-hidden text-white selection:bg-[#EBB048] selection:text-[#8B2510]`}>
        ...
        <div className="max-w-7xl mx-auto bg-[#f3db9a00] backdrop-blur-sm border-2 border-black rounded-xl px-4 md:px-6 py-3 flex justify-between items-center shadow-[0px_4px_0px_0px_rgba(0,0,0,0.3)]">
          <div className="flex items-center gap-3">
            <div className="bg-[#ced5ca] border-2 border-black rounded-lg overflow-hidden flex items-center justify-center shadow-[3px_3px_0_0_#000]">
               <Image src={logoPath} alt="Logo" width={40} height={40} className="object-contain" />
            </div>
        ...
        <main className="max-w-[1400px] mx-auto w-full px-4 md:px-8 lg:px-16">
          ...
          <section className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-center py-12 md:py-10">
            <div className="order-2 lg:order-1 relative flex justify-center items-center group">
              ...
              <div className="relative z-10 w-full max-w-[420px] aspect-[3/4] bg-[#af005e] rounded-t-full rounded-b-[2.5rem] border-dashed border-4 border-[#e0f374] shadow-[20px_20px_0px_0px_rgba(0,0,0,0.4)] overflow-hidden transition-all duration-500 group-hover:shadow-[30px_30px_0px_0px_rgba(0,0,0,0.3)]">
                <video autoPlay loop muted playsInline className="w-full h-full object-cover grayscale contrast-[1.2] brightness-[0.9] transition-all duration-700 group-hover:grayscale-0 group-hover:contrast-100">
            ...
            <div className="order-1 lg:order-2 flex flex-col items-center lg:items-start text-center lg:text-left space-y-8">
              <h1 className="text-6xl md:text-7xl lg:text-[5.5rem] font-black text-white leading-[0.9] tracking-tighter drop-shadow-xl">
                Take your <br className="hidden lg:block" /> designs to <br className="hidden lg:block" /> <span className="text-[#EBB048] italic">the next level</span>
              </h1>
              <div className="max-w-lg border-l-0 lg:border-l-4 border-[#EBB048] lg:pl-8 py-2">
                <TextGenerateEffect words="Experience real-time messaging that feels natural. Designed for creators, built for speed, and styled for you." className="text-lg md:text-2xl font-medium text-white/90 leading-relaxed" />
              </div>
              <Link href="/sign-up">
                <button className="px-12 py-5 bg-[#EBB048] text-black font-black text-xl border-2 border-black rounded-2xl shadow-[8px_8px_0px_0px_#000] active:translate-x-[6px] active:translate-y-[6px] active:shadow-none transition-all uppercase italic tracking-tighter">Explore Now</button>
              </Link>
            </div>
          </section>
          ...
          <section className="py-20">
            <div className="flex items-center gap-4 mb-16">
              ...
              <h2 className="text-3xl font-black text-white uppercase italic tracking-widest" style={{ fontFamily: 'var(--font-bokor)' }}>Our Services</h2>
              <div className="h-[2px] flex-1 bg-white/20"></div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pb-10">
              {[{ t: "Instant Sync", d: "Messages delivered in under 100ms across global edges.", icon: "⚡" }, { t: "Secure Core", d: "Military grade AES-256 bit encryption as standard.", icon: "🔐" }, { t: "Open API", d: "Build your own custom integrations effortlessly.", icon: "🛠️" }].map((item, i) => (
      ...
      <footer className="relative z-20 w-full shrink-0 border-t-2 border-black bg-[#5a1608]/95 backdrop-blur-xl mt-auto">
        <div className="max-w-7xl mx-auto px-6 py-2 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-3">
            <Image src={logoPath} alt="Logo" width={20} height={20} className="rounded-sm" />
            <span className="text-lg font-black text-white" style={{ fontFamily: 'var(--font-bokor)' }}>Post<span className="text-[#EBB048]">Pilot</span></span>
            <span className="hidden md:block h-4 w-[1px] bg-white/20"></span>
            <span className="text-white/40 text-[9px] font-bold tracking-[0.2em] uppercase">© 2026 C.dOT Protocol</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <span className="text-[#EBB048] font-black tracking-[0.1em] uppercase text-[9px] flex items-center gap-2" style={{ fontFamily: 'var(--font-bokor)' }}>
              Crafted by <span className="font-mono text-[11px] text-white">Disha</span>
            </span>
